...
module tb_cache_drain_overlap;
  ...
  integer backend_latency=0,backend_count=0;
  logic backend_pending=0;
  always_ff @(posedge mem_clk) begin
    if (mem_rst) begin
      vram_ack<=0;memory_seen<=0;memory_accesses<=0;backend_pending<=0;backend_count<=0;
      vram_rdata<=16'hffff;vram_rdata64<=0;
    end else begin
      vram_ack<=0;
      if(!vram_req) memory_seen<=0;
      if(vram_req && !memory_seen) begin
        memory_seen<=1;backend_pending<=1;backend_count<=backend_latency;
      end
      if(backend_pending) begin
        if(backend_count!=0) backend_count<=backend_count-1;
        else begin
          backend_pending<=0;memory_accesses<=memory_accesses+1;vram_ack<=1;
          if(vram_we) begin
            ...
            vram_rdata<=memory[vram_addr];
            vram_rdata64<={memory[{vram_addr[18:2],2'b11}],memory[{vram_addr[18:2],2'b10}],memory[{vram_addr[18:2],2'b01}],memory[{vram_addr[18:2],2'b00}]};
          end
        end
      end
    end
  end
  integer clocks=0,started=0,checks=0;
  always @(posedge clk) clocks++;
  initial begin #100_000_000; $fatal(1,"test timeout");end
  ...
  initial begin
    ...
    // Re-prime both caches before holding a physically unrelated write.
    access(LOCAL_CYCLE_WORD_READ,32'hff820000,0,0,memory[8192]);
    access(LOCAL_CYCLE_WORD_READ,32'hff820000,0,1,memory[8192]);
    backend_latency=200;
    access(LOCAL_CYCLE_WORD_WRITE,32'hff840100,16'h1234,0,0);
    wait(dut.wq_drain_busy_q);
    for(int iaq=0;iaq<2;iaq++) begin
      started=clocks;
      access(LOCAL_CYCLE_WORD_READ,32'hff820000,0,1'(iaq),memory[8192]);
      if(clocks-started>20 || !dut.wq_drain_busy_q) $fatal(1,"cached read waited for unrelated drain: %0d clocks",clocks-started);
      checks++;
    end
    started=clocks;
    access(LOCAL_CYCLE_WORD_WRITE,32'hff840200,16'h5678,0,0);
    if(clocks-started>20 || !dut.wq_drain_busy_q) $fatal(1,"posted write waited for drain");
    checks++;
    // Queue pressure, wraparound and same-line merges with delayed commit.
    for(int i=0;i<96;i++) access(LOCAL_CYCLE_WORD_WRITE,32'hff850000+32'(i*16),16'(i)^16'hbabe,0,0);
    for(int i=0;i<96;i++) access(LOCAL_CYCLE_WORD_READ,32'hff850000+32'(i*16),0,0,16'(i)^16'hbabe);
    access(LOCAL_CYCLE_WORD_READ,32'hff840100,0,0,16'h1234);
    access(LOCAL_CYCLE_WORD_READ,32'hff840200,0,0,16'h5678);
    checks+=98;
    // Both cache types must observe a pending write to their cached word.
    for(int iaq=0;iaq<2;iaq++) begin
      access(LOCAL_CYCLE_WORD_READ,32'hff820000,0,1'(iaq),memory[8192]);
      access(LOCAL_CYCLE_WORD_WRITE,32'hff820000,16'h9876+16'(iaq),0,0);
      started=clocks;
      access(LOCAL_CYCLE_WORD_READ,32'hff820000,0,1'(iaq),16'h9876+16'(iaq));
      if(clocks-started<50) $fatal(1,"conflicting read bypassed uncommitted write");
      checks++;
    end
    backend_latency=0;
    $display("PASS cache/drain overlap checks=%0d; queue pressure, I/D invalidation and read-after-write ordering",checks);
    $finish;
  end
endmodule
