module IKA32010 (
    ...
    //flag
    input   wire            i_BIO_n,
    // One master-clock pulse when a BIOZ instruction consumes the sampled
    // BIO input.  External board logic uses this to acknowledge a timed BIO
    // event; it does not alter the TMS32010's architectural behaviour.
    output  wire            o_BIO_POLL,
    //interrupt
    input   wire            i_INT_n
);
...
// MPYK carries a signed 13-bit constant in opcode bits 12:0.  The generic
// immediate write-bus source is only eight bits wide (for LACK/LARK), so
// routing MPYK through it silently discarded bits 12:8.
wire    [15:0]  mul_op1 = mul_op1_source_sel
                         ? reg_wrbus
                         : {{3{if_opcodereg[12]}}, if_opcodereg[12:0]};
reg             mul_en;
...
reg             stk_pc_next;  // direct CALL returns past its immediate word
reg             stk_pop, stk_push;

IKA32010_stack u_stack (
    .i_EMUCLK(i_EMUCLK), .i_RST_n(i_RS_n), .i_CEN(cyc_ncen),
    .i_PUSH(stk_push), .i_POP(stk_pop),
    .i_DIN(stk_data_sel ? (stk_pc_next ? if_pc_next : if_pc)
                         : reg_wrbus[11:0]), .o_DOUT(stk_output)
);
...
assign o_BIO_POLL = i_RS_n && cyc_ncen && ex_state && !bio_n &&
                    (if_opcodereg == 16'hF600) && (ex_inst_cycle == 2'd0);
        ...
        casez(if_opcodereg)
            ...
            16'b1111_1000_0000_0000: begin
                if(ex_inst_cycle == 2'd0) begin
                    ...
                    //deny interrupt request
                    if_opcodereg_force_iack = NO; 
                    // if_pc addresses CALL's immediate operand here.  The
                    // architectural return address is the following word.
                    stk_push = YES; stk_data_sel = STACK_DATA_PC;
                    stk_pc_next = YES;
                end
                else if(ex_inst_cycle == 2'd1) begin
